...
router
  ...
  .post(isLoggedIn, (req, res) => {
    const { name, typeOfMeeting, language, schedule, time, city, description } =
      req.body;
...
router
  ...
  .get(isLoggedIn, (req, res) => {
    ...
    Meeting.findById(id)
      .populate("host")
      .then((meeting) => {
        if (meeting.host._id != req.session.userId)
  ...
  .post(isLoggedIn, (req, res) => {
    ...
    Meeting.findByIdAndUpdate(id, {
      ...
      .catch((error) => {
        res.render("meetings/meeting-edit");
      });
  });
...
router.post("/:id/join", isLoggedIn, (req, res, next) => {
  Meeting.findById(req.params.id)
    .then((meeting) => {
      if (
        meeting.attendees.includes(req.session.userId) ||
        req.session.userId == meeting.host
      ) {
        res.redirect(`/meetings/${meeting._id}`);
      } else {
        Meeting.findByIdAndUpdate(req.params.id, {
          ...
          .then((meeting) => {
            User.findByIdAndUpdate(req.session.userId, {
              $push: { meetingsAttended: req.params.id },
            }).then((user) => {
              res.redirect(`/meetings/${meeting._id}`);
            });
          })
